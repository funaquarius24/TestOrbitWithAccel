...
%     Y_d0(1) = x(ind_time);
    ...
    data(i).position = [x(ind_time) + randi(50), y(ind_time) + randi(50), z(ind_time) + randi(50)]; %#ok<SAGROW>
    time = tspan(ind_time);
...
for i=1:numDebris
    data(i).scatter = scatter3(data(i).position(1), data(i).position(2), data(i).position(3), 100, 'filled');
end
for t=1:length(x)
    if mod(t, 100) == 0
        disp(t)
